...
// Schema para resposta da configuração MySQL
...
// Schema para resposta de reconexão MySQL
const mysqlReconnectSchema = z.object({
  status: z.string(),
  message: z.string(),
...
// Schema para resposta do status da conexão MySQL
  ...
  definition: {
    ...
    components: {
      schemas: {
        ...
        MySQLConfig: zodToJsonSchema(mysqlConfigSchema),
        MySQLReconnect: zodToJsonSchema(mysqlReconnectSchema)
      }
    }
  },
  apis: ['./src/server.ts']
 ...
 * /mysql/config:
 *   get:
 *     summary: Configuração da conexão MySQL
 *     description: Retorna as configurações atuais da conexão MySQL (sem expor senha)
 *     tags: [Database]
 *     responses:
 *       200:
 *         description: Configurações da conexão MySQL
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                   format: date-time
 */
app.get('/mysql/config', (req: express.Request, res: express.Response) => {
  const configInfo = {
    host: connection.config.host || 'localhost',
    user: connection.config.user || 'root',
    database: connection.config.database || 'bd_tasks',
    connectionStatus: 'connected', // Assumindo que está conectado se chegou até aqui
    timestamp: new Date().toISOString()
  };
  
  // Validar resposta
  const validatedResponse = mysqlConfigSchema.parse(configInfo);
  res.status(200).json(validatedResponse);
});
 ...
 * /mysql/reconnect:
 *   post:
 *     summary: Reconectar com MySQL
 *     description: Força uma nova conexão com o banco de dados MySQL
 *     tags: [Database]
 *     responses:
 *       200:
 *         description: Reconexão bem-sucedida
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                 message:
 *                   type: string
 *                   example: 'MySQL reconectado com sucesso!'
 *                 previousState:
 *                   type: string
 ...
 *                   format: date-time
 *       500:
 *         description: Erro na reconexão
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/ErrorResponse'
 */
app.post('/mysql/reconnect', (req: express.Request, res: express.Response) => {
  ...
  connection.connect((err) => {
    ...
    if (err) {
      console.error('❌ Erro ao reconectar com MySQL:', err);
      res.status(500).json({
        error: 'Connection Error',
        message: `Erro ao reconectar com MySQL: ${err.message}`,
        timestamp
      });
    } else {
      console.log('✅ MySQL reconectado com sucesso!');
      
      const reconnectResponse = {
        status: 'success',
        message: 'MySQL reconectado com sucesso!',
        previousState,
        newState: 'connected',
        timestamp
      };
      ...
      // Validar resposta
      const validatedResponse = mysqlReconnectSchema.parse(reconnectResponse);
      res.status(200).json(validatedResponse);
    }
  });
});
 ...
 *   get:
 *     summary: Detalhes completos da conexão MySQL
 *     description: Retorna informações detalhadas sobre a conexão atual com MySQL
 *     tags: [Database]
 *     responses:
 *       200:
 *         description: Detalhes da conexão MySQL
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                   format: date-time
 */
app.get('/mysql/connection-details', (req: express.Request, res: express.Response) => {
  // Executar query para obter informações do servidor
  connection.query('SELECT VERSION() as serverVersion, NOW() as serverTime', (err, results: any) => {
    if (err) {
      res.status(500).json({
        error: 'Query Error',
        message: `Erro ao obter detalhes da conexão: ${err.message}`,
        timestamp: new Date().toISOString()
      });
      return;
    }

    const serverInfo = Array.isArray(results) && results.length > 0 ? results[0] : {};
    ...
    const connectionDetails = {
      ...
      connectionTime: serverInfo.serverTime || null,
      timestamp: new Date().toISOString()
    };
 ...
 *   get:
 *     summary: Testar conexão MySQL atual
 *     description: Executa um teste simples para verificar se a conexão MySQL está ativa
 *     tags: [Database]
 *     responses:
 *       200:
 ...
 *                 message:
 *                   type: string
 *                   example: 'Conexão MySQL testada com sucesso!'
 *                 testResult:
 *                   type: number
 *                   example: 1
 *                 responseTime:
 ...
 *                   format: date-time
 *       500:
 *         description: Erro no teste de conexão
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/ErrorResponse'
 */
app.get('/mysql/test-connection', (req: express.Request, res: express.Response) => {
  const startTime = Date.now();
  
  connection.query('SELECT 1 as test', (err, results: any) => {
    const endTime = Date.now();
    const responseTime = endTime - startTime;
    const timestamp = new Date().toISOString();
    
    if (err) {
      console.error('❌ Erro no teste de conexão MySQL:', err);
      res.status(500).json({
        error: 'Connection Test Error',
        message: `Erro no teste de conexão MySQL: ${err.message}`,
        responseTime,
        timestamp
      });
    } else {
      const testResult = Array.isArray(results) && results.length > 0 ? results[0].test : null;
      
      console.log(`✅ Teste de conexão MySQL bem-sucedido - ${responseTime}ms`);
      res.status(200).json({
        status: 'success',
